...
import { DEFAULTS, ENV, NRK } from '../constants'

const NRK_API = NRK.API_BASE_URL
const NRK_FETCH_DELAY_MS = ENV.NRK_FETCH_DELAY_MS
const NRK_FETCH_DELAY_JITTER = Math.min(
	1,
	Math.max(0, ENV.NRK_FETCH_DELAY_JITTER)
)
...
async function getPlaybackUrl(
	...
	for (const endpoint of endpoints) {
		const { status, body } = await fetchJson<PlaybackManifest>(
			endpoint,
			options
		)
...
const VERBOSE = process.env.BACKFILL_VERBOSE === DEFAULTS.BACKFILL_VERBOSE
...
async function fetchEpisodesUntilKnown(
	...
	while (url && !reachedKnown) {
		...
		if (VERBOSE) {
			console.log(
				`  ${seriesId}: incremental catalog ${collected.length} episodes`
			)
		}
...
export async function fetchPlaybackUrlsBatch(
	...
	if (valid.length < batch.length && VERBOSE) {
		console.warn(
			`  Skipped ${batch.length - valid.length} non-playable in batch`
		)
	}
	return valid
}
