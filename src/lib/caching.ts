...
const SYNC_INTERVAL_HOURS = DEFAULTS.SYNC_INTERVAL_HOURS
const MAX_BYTES = DEFAULTS.MAX_SERIES_BYTES
...
async function updateFetch(existing: Series): Promise<Series | null> {
	const lockToken = await storage.acquireSeriesLock(
		existing.id,
		SERIES_FETCH_LOCK_TTL_SEC
	)
	if (!lockToken) {
		console.log('[feed] Stale refresh skipped (lock active):', existing.id)
		return existing
	}

	try {
		const knownEpisodeIds = new Set(existing.episodes.map((ep) => ep.id))
		const updatesCatalog = await getSeriesCatalogUpdates(
			existing.id,
			knownEpisodeIds
		)
		...
		for (
			let i = 0;
			i < updatesCatalog.episodeResources.length;
			i += updateBatchSize
		) {
			const batch = updatesCatalog.episodeResources.slice(
				i,
				i + updateBatchSize
			)
		...
		await storage.releaseSeriesLock(existing.id, lockToken)
	}
}
